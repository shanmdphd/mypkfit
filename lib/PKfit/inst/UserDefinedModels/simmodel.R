...
par<-data.frame(Parameter=c("Total_subject#","ka","kel","Vd","F"),
                Initial=c(12,0.361,0.132,0.17,0.85))  ### (1) set default PK parameters for sim here
        cat("\n")       
        ### print(par, row.names=F)   ### not working in source();
        print(par)
        cat("\n")
        Subject<-par[1,2]
        par1   <-par[2,2]
...
sim.type<- 1   ### (6) select general simulation (1) or monte-carlo sim (2) here
...
err.type<- 2   ### (7) select error type for general simulation
pick<-err.type
      ...
      if (err.type ==1){
          ...
          for(i in 1:Subject)  {
           ...
           cat("*******************************************************\n\n")      
             ...
             good<-add.err(good)   ### add random errors to integrated Cp
  ...
  par.err<-data.frame(Parameter=c("ka","kel","Vd","F"),error_factor=c(.15,.15,.15,.15))  ### (8) change error factors for general sim.
         ...
         par(mfrow=c(2,1),las=1)
           ...
           x<-C1.lsoda[2:(length(time)+1),1]
           y<-good
           plotting.sim(i,x,y,xaxis,yaxis,MD) ### ;cat("\n\n");readline("  Press Enter to continue....");cat("\n\n")
     } 
   }
}
...
pcik<-err.type
     ...
     for(i in 1:Subject)  {
       ...
       for (j in 1:re){
             ...
             ### dev.new()    
             PKindex[[i]]<-montecarlo(C1.lsoda,time,i,re,xaxis,yaxis)
             ### dev.new()
             montecarlo2(PKindex[[i]],i,re,xaxis,yaxis)
     }  
  }
### run() ### req. if running on RStudio; don't know why it is so.
