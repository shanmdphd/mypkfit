...
Dose = 500
   ...
   defun<- function(time, y, parms) { 
     dCp1dt <- -parms["Vm"]*y[1]/(parms["Km"]+y[1])-parms["k12"]*y[1]+parms["k21"]*y[2] 
     dCp2dt <-  parms["k12"]*y[1]-parms["k21"]*y[2]
     list(c(dCp1dt,dCp2dt)) 
   } 

   modfun <- function(time,Vm,Km,k12,k21,Vd) { 
     out <- lsoda(y=c(Dose/Vd,0),c(0,time),defun,parms=c(Vm=Vm,Km=Km,k12=k12,k21=k21,Vd=Vd),
                  rtol=1e-6,atol=1e-6) 
     ### plot(out)
     out[-1,2] 
   }   
     ...
     objfun <- function(par) {
        out <- modfun(PKindex$time, par[1], par[2], par[3], par[4], par[5])
     ...
     opt <- optim(c(0.1,2,0.3,0.2,5),objfun,method="Nelder-Mead")         ### change initial values if necessary
     nameopt<-c("Vm","Km","k12","k21","Vd")
     ...
     fm<-nlsLM(conc ~ modfun(time,Vm,Km,k12,k21,Vd),data=PKindex,
         start=list(Vm=opt$par[1],Km=opt$par[2],k12=opt$par[3],k21=opt$par[4],Vd=opt$par[5]),
         control=nls.lm.control(maxiter=500))
     coef<-data.frame(coef(fm)["k21"])     
