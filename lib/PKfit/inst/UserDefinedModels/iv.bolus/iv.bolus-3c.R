...
Dose<-500   ### assign given dose here
   ...
   modfun1 <- function(time,kel,k12,k21,k13,k31,Vd) { 
     out <- lsoda(y=c(Dose/Vd,0,0),c(0,time),defun,parms=c(kel=kel,k12=k12,k21=k21,k13=k13,k31=k31,Vd=Vd),
                  rtol=1e-06,atol=1e-06)
     # par(mfrow=c(2,2),las=1,recording=TRUE)
     # plot(out)
     out[-1,2] 
   }   

     objfun <- function(par) {
        out <- modfun1(PKindex$time, par[1], par[2], par[3], par[4], par[5], par[6])
     ...
     opt <- optim(c(0.1,0.2,0.2,0.3,0.3,10),objfun,method="Nelder-Mead",control=list(maxit=5000)) ### change initial values if necessary
     ...
     fm<-nlsLM(conc ~ modfun1(time,kel,k12,k21,k13,k31,Vd),data=PKindex,
         start=list(kel=opt$par[1],k12=opt$par[2],k21=opt$par[3],k13=opt$par[4],k31=opt$par[5],Vd=opt$par[6]))###,trace=TRUE,
         ## start=list(kel=0.002443,k12=0.041628,k21=0.051434,k13=0.041628,k31=0.000512,Vd=7.237))###,trace=TRUE,
         ### nls.control(maxiter=5000,tol=1e-06,minFactor=1/1024/1024),algorithm = "port",lower=c(0,0,0,0,0,0))
