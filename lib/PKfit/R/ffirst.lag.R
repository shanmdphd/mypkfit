...
if(MD){
   if (MMe){
   if (file.exists("ffirst_lag_mm_md.csv") || file.exists("sfirst_lag_mm_md.csv")){     ### since v1.3.7
   par.init<-read.par.from.stored(MD,"ffirst_lag_mm_md.csv","sfirst_lag_mm_md.csv")}
   else{
   par.init<-data.frame(Parameter=c("Dose","Tau","#Dose","Tlag","ka","Vm","Km","Vd"),Initial=c(0,0,0,0,0,0,0,0))
   ...
   Tau       <-par.init[2,2]
   nDose     <-par.init[3,2]
   Tlag_time <-par.init[4,2]
   par1      <-par.init[5,2]
   par2      <-par.init[6,2]
   par3      <-par.init[7,2]
   par4      <-par.init[8,2]
   write.csv(par.init,file="ffirst_lag_mm_md.csv",row.names=FALSE)
} 
   else{
   if (file.exists("ffirst_lag_md.csv") || file.exists("sfirst_lag_md.csv")){     ### since v1.3.7
   par.init<-read.par.from.stored(MD,"ffirst_lag_md.csv","sfirst_lag_md.csv")}
   else{
   par.init<-data.frame(Parameter=c("Dose","Tau","#Dose","Tlag","ka","kel","Vd"),Initial=c(0,0,0,0,0,0,0))
   par.init<-edit(par.init)}
   ### }  ### since v1.3.7}
   par.init<-check.para(par.init)
   Dose      <-par.init[1,2]
   ...
   par2      <-par.init[6,2]
   par3      <-par.init[7,2]
   write.csv(par.init,file="ffirst_lag_md.csv",row.names=FALSE)
   }
        cat("\n")
   ...
   if (MMe){
     if (file.exists("ffirst_lag_mm.csv") || file.exists("sfirst_lag_mm.csv")){     ### since v1.3.7
     par.init<-read.par.from.stored(MD,"ffirst_lag_mm.csv","sfirst_lag_mm.csv")}
     else{
     ...
     par3      <-par.init[5,2]
     par4      <-par.init[6,2]
     write.csv(par.init,file="ffirst_lag_mm.csv",row.names=FALSE)
   } 
   else{
     if (file.exists("ffirst_lag.csv") || file.exists("sfirst_lag.csv")){     ### since v1.3.7
     par.init<-read.par.from.stored(MD,"ffirst_lag.csv","sfirst_lag.csv")}
     else{
     ...
     par2<- par.init[4,2]
     par3<- par.init[5,2]
     write.csv(par.init,file="ffirst_lag.csv",row.names=FALSE)
   }
        cat("\n")
        print(par.init, row.names=F);cat("\n")
   ...
   for( i in 1:length(unique(PKindex$Subject)))  {
     ...
     if (MMe) {
        ...
        fm <<- fm
        sink(zz,split=TRUE)
        this.model<-"--- model selection: one-compartment, extravascular PK model\n   with 1st-ordered lag-time abs. and M-M elim.\n\n"
