...
if(MD){
   if (!MMe) {
      ...
      par1   <-par[5,2]
      par2   <-par[6,2]
       write.csv(par,file="sbolus1_md.csv",row.names=FALSE)
  }
   else{
      ...
      cat("\n")       
      print(par, row.names=F)
      write.csv(par,file="sbolus1_mm_md.csv",row.names=FALSE)
      cat("\n")
      Subject<-par[1,2]
      Dose   <-par[2,2]
      Tau    <-par[3,2]
...
### print(Tau);cat("\n");print(nDose);readline()    ### for debugging; move one "}" to upper... -YJ
   ...
   if (!MMe) {
      ...
      par1<-par[3,2]
      par2<-par[4,2]
      write.csv(par,file="sbolus1.csv",row.names=FALSE)
  }
   else{
      ...
      par2<-par[4,2]
      par3<-par[5,2]
      write.csv(par,file="sbolus1_mm.csv",row.names=FALSE)
}
...
### 
     ...
     if (pick ==1){          ### we will use this module for tdm too with mutiple-dose scheme.
         ...
         if (!MMe){
           ...
           for( i in 1:Subject)  {
             ...
             kel<- err_types(pick,par1,factor1)  # reset pick = 1, 2, 3 or 4 ('0' is zero error & excluded)
   ...
   ### if(MD) sone.iv.route.MD() else sone.iv.route.SD()
   run(first.run=FALSE)
} 
