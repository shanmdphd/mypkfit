...
iv.bolus.3c.mm<- function(){
options(warn=-1)
### library(nls2)  ### may req. v1.3.7
...
Dose<-500   ### assign given dose here
   ...
   modfun <- function(time,Vm,Km,k12,k21,k13,k31,Vd) { 
     ...
     ### the following line uses 'fm<<-nlsML(...)' is a must
     fm<<-nlsLM(conc ~ modfun(time,Vm,Km,k12,k21,k13,k31,Vd),data=PKindex, weights=(1/conc^2), ### verify this weighting method later... -YJ
...
### if(.Platform$OS.type=="windows" && .Platform$GUI=="Rgui") windows(record=TRUE) ### will fail to save plots as a pdf!
...
# xx<-predict(as.lm.nls(fm),interval="prediction",level=0.95)
xx<-as.data.frame(xx)
...
plot(x,y,log="y",type='p',main="Drug Plasma Conc. vs. Time Curve",
     ...
     font.lab=2,cex.lab=1,cex.axis=1,cex.main=1)
### polygon(c(x,rev(x)),c(xx$lwr,rev(xx$upr)),border=FALSE,col="grey75")
### polygon(c(xy1spline$x,rev(xy2spline$x)),c(xy1spline$y,rev(xy2spline$y)),border=FALSE,col="grey75") ### omit this for semi-log plot
points(x,y,pch=15,col="black")
lines(x,predict(fm,list(time=x)),type="l",lty=1,col="firebrick3",lwd="2")
